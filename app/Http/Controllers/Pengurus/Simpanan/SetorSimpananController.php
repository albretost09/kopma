...
class SetorSimpananController extends Controller
{
    ...
    {
        return view('pages.pengurus.setor-simpanan.index');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->jenis_simpanan == 'Pokok') {
            if ($simpananPokok) {
                return redirect()->route('pengurus.setor-simpanan.index')->with('error', 'Anda sudah memiliki simpanan pokok');
            }
        } else {
            if (!$simpananPokok) {
                return redirect()->route('pengurus.setor-simpanan.index')->with('error', 'Anda belum memiliki simpanan pokok');
            }
        }
        ...
        if ($result) {
            return redirect()->route('pengurus.riwayat-simpanan.index')->with('success', 'Simpanan berhasil disetor');
        } else {
            return redirect()->route('pengurus.riwayat-simpanan.index')->with('error', 'Simpanan gagal disetor');
        }
    }
}
